...
fid=fopen('debugsomestuff.csv','w');
[croplistvector,cropnumlist]=N2OAnalysis_cropnames;

for jclv=1:length(croplistvector)
    
croplist=croplistvector(jclv);

%croplist={'rice_irrigated_monfredamask','rice_rainfed_monfredamask'};
cropnumlist=[1 ];
...
fprintf(fid,'%s,%f,%f\n',cropname,sum(metanv.*metaav),sumtotalNapp);
end
crash
