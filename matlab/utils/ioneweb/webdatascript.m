...
%         ['outfiles/area/' ThisCropName '_overlay/file/overlay.png'],'Alpha',rot90(fliplr(yield))/125+.2*(rot90(fliplr(area))>0));
% 
%     imwrite(ind2rgb(round(rot90(fliplr(area))),AREACMAP),...
%         ['outfiles/yield/' ThisCropName '_overlay/file/overlay.png'],'Alpha',rot90(fliplr(area))/125+.2*(rot90(fliplr(yield))>0));
    ...
    layeredkmllegend(area,['Area: ' ThisCropName ' (percentage)'],'jet','outfiles/area/',[ThisCropName '_area'],0,amax);

    layeredkmllegend(yield,['Yield: ' ThisCropName ' (tonnes per ha)'],'jet','outfiles/yield/',[ThisCropName '_yield'],0,ymax);
...
for i=1:length(ANC)
    ...
    if (i-1>=0)&&(mod(i-1,20)==0)
        if (i>10)
            fprintf(fid,'</table>');
        end
        for j=1:length(letters)-1
            fprintf(fid,['<a href="#' letters(j) '">' upper(letters(j)) ' </a>']);
        end
fprintf(fid,['<table cellpadding="4" cellspacing="0">'...
    '<tr><td>Crop</td>'...
    '<td>PNG Map: Area</td>'...
    '<td>PNG Map: Yield</td>'...
    '<td>KML Map: Area</td>'...
...
fixedname=regexprep(strrep(strrep(strrep(strrep(APNG(i).name, '.png', ''),'for',' (for silage)'),'nes',' (other)'),'_area',''),'(\<[a-z])','${upper($1)}');

    if (mod(i,2)==1)
        if strcmp(APNG(i).name(1),letters(marker))
            fprintf(fid,['<tr>'...
                '<td><a name="' letters(marker) '">' fixedname '</a></td>'...
                '<td><a href="area/' APNG(i).name '">' bytestostr(APNG(i).bytes) '</a></td>'...
                '<td><a href="yield/' YPNG(i).name '">' bytestostr(YPNG(i).bytes) '</a></td>'...
                '<td><a href="area/' AKMZ(i).name '">' bytestostr(AKMZ(i).bytes) '</a></td>'...
                '<td><a href="yield/' YKMZ(i).name '">' bytestostr(YKMZ(i).bytes) '</a></td>'...
                '</tr>']);
            marker=marker+1;
        else
            fprintf(fid,['<tr>'...
