function [BinMatrix,TempBins,WaterBins,ClimateDefs,CDS]=...
    MakeClimateSpace3rdCategory(Temp,Water,TempBins,WaterBins,CategoryMap,Categories);
% MAKECLIMATESPACE - make a climate space
%
%  SYNTAX
%     [BinMatrix,TempBins,WaterBins,ClimateDefs,CDS]=MakeClimateSpace(Temp,Water
%     ,TempBins,WaterBins,CategoryMap,Categories);
%
%   INPUTS:
...
if ~isequal(size(Temp),size(Water)) | ~isequal(size(Temp),size(CategoryMap))
...
for mC=1:length(Categories)
    ...
    for mW=1:NW
        for mT=1:NT
            ...
            ClimateBinNumber= (mC-1)*(NW*NT)+ (mW-1)*NT+mT;
            ...
            % who fits?
            jj=find( T >= Tmin & T <Tmax & W >= Wmin & W <Wmax & C==c);
            ...
            CDS(ClimateBinNumber).Category=c;
        end
    end
end
