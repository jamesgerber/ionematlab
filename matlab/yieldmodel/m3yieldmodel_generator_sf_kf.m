...
for bin = binlistvector
    ...
    if ii > 20
                        ...
                        elseif i == 1
                            ...
                            switch modelnumber
                                case 1 % VL ELM: 1 nutrient + irr
                                    ...
                                    yw = sqrt(w).*yield_bin;
                                    agmgmtw = @(b,x) double(sqrt(w).*...
                                        agmgmt_elm_irrintmodel_nutoneirron_sf(b,x));
                        ...
                        elseif i == 1
                            ...
                            switch modelnumber
                                ...
                                case 1 % VL ELM: 2 nutrients + irr
                                    ...
                                    yw = sqrt(w).*yield_bin;
                                    agmgmtw = @(b,x) double(sqrt(w).*...
                                        agmgmt_elm_irrintmodel_nuttwoirron_sf(b,x));
                        ...
                        elseif i == 1
                            ...
                            switch modelnumber
                                ...
                                case 1 % VL ELM: 3 nutrients + irr
                                    ...
                                    yw = sqrt(w).*yield_bin;
                                    agmgmtw = @(b,x) double(sqrt(w).*...
                                        agmgmt_elm_irrintmodel_nutthreeirron_sf(b,x));
        ...
        % assign chosen cb to IRR functions
        assign.cb = cb;
        [~] = agmgmt_elm_irrintmodel_nutoneirron_sf(0,0,assign);
        [~] = agmgmt_elm_irrintmodel_nuttwoirron_sf(0,0,assign);
        [~] = agmgmt_elm_irrintmodel_nutthreeirron_sf(0,0,assign);
...
w = arealist ./ sum(arealist); %[w(1:60) w(62:80) w(82:100)]
...
for c = 1:length(cropOS.xlist)
    cb = cropOS.xlist{c};
    if isstr(cb)
        if str2num(cb(3)) == 1 % only look up b_K if K2O is an explan variable
            ...
            switch numinputs
                case 1
                    output{c+1,10} = bFitw(2);
                case 2
                    output{c+1,10} = bFitw(3);
                case 3
                    output{c+1,10} = bFitw(4);
                case 4
                    output{c+1,10} = bFitw(5);
            end
        end
    end
end
...
for c = 1:length(cropOS.xlist)
    ...
    if isstr(cb)
        ...
        if str2num(cb(3)) == 1
            ...
            switch othernuts
                case 0
                    output{c+1,13} = abs(bFitw(1));
                case 1
                    output{c+1,13} = abs(bFitw(2));
                case 2
                    output{c+1,13} = abs(bFitw(3));
            end
        end
    end
end
output{102,13} = slopeGK;
...
for c = 1:length(cropOS.xlist)
    ...
    if isstr(cb)
        ...
        if str2num(cb(4)) == 1
            ...
            switch othernuts
                case 0
                    output{c+1,14} = bFitw(1);
                case 1
                    output{c+1,14} = bFitw(2);
                case 2
                    output{c+1,14} = bFitw(3);
                case 3
                    output{c+1,14} = bFitw(4);
            end
        end
    end
end
...
output{1,16} = 'bin_r2';
for c = 1:100; % for cotton: 1:length(cropOS.statslist(:,1))
