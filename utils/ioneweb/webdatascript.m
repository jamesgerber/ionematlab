...
%    for i=3
    ...
    gridtoimage(area,AREACMAP,yield,[ThisCropName 'overlaya.png']);
...
%   MakeGlobalOverlay(area(area>1),'area2',[0 100],['outfiles/area/' ThisCropName '_overlay/file/overlay.png'],0.5);
 ...
 MakeGlobalOverlay(yield,'brightyield',[.98],[ThisCropName 'overlayy.png'],0.5);
    ...
    layeredkmllegend(imread([ThisCropName 'overlaya.png']),['Area ' ThisCropName]);
    
    layeredkmllegend(imread([ThisCropName 'overlayy.png']),['Area ' ThisCropName]);
...
fid=fopen('ionedocs.html','w');
letters='abcdefghjklmnopqrstvwy ';
fprintf(fid,['<html>\n'...
'<head>'...
  '<title>GLI Documents</title>'...
'</head>']);
for i=1:length(letters)-1
    fprintf(fid,['<a href="' letters(i) '">' upper(letters(i)) ' </a>']);
end
...
for i=1:length(ANC)
if strcmp(APNG(i).name(1),letters(marker))
    fprintf(fid,['<tr>'...
        '<td><a name="' letters(marker) '">' strrep(strrep(strrep(APNG(i).name, '.png', ''),'for',' (for silage)'),'nes',' (other)') '<\a></td>'...
        '<td><a href="area/' APNG(i).name '">' bytestostr(APNG(i).bytes) '</a></td>'...
        '<td><a href="area/' YPNG(i).name '">' bytestostr(YPNG(i).bytes) '</a></td>'...
        '<td><a href="area/' AKMZ(i).name '">' bytestostr(AKMZ(i).bytes) '</a></td>'...
        '<td><a href="area/' YKMZ(i).name '">' bytestostr(YKMZ(i).bytes) '</a></td>'...
        '<td><a href="area/' AGRD(i).name '">' bytestostr(AGRD(i).bytes) '</a></td>'...
        '<td><a href="area/' YGRD(i).name '">' bytestostr(YGRD(i).bytes) '</a></td>'...
        '<td><a href="area/' ANC(i).name '">' bytestostr(ANC(i).bytes) '</a></td>'...
        '</tr>']);
    marker=marker+1;
else
    fprintf(fid,['<tr>'...
        '<td>' strrep(strrep(strrep(APNG(i).name, '.png', ''),'for',' (for silage)'),'nes',' (other)') '</td>'...
        '<td><a href="area/' APNG(i).name '">' bytestostr(APNG(i).bytes) '</a></td>'...
        '<td><a href="area/' YPNG(i).name '">' bytestostr(YPNG(i).bytes) '</a></td>'...
        '<td><a href="area/' AKMZ(i).name '">' bytestostr(AKMZ(i).bytes) '</a></td>'...
        '<td><a href="area/' YKMZ(i).name '">' bytestostr(YKMZ(i).bytes) '</a></td>'...
