function [XXXBinEdges,YYYBinEdgesCell,xbins,ybins,ContourMask]= ...
    CalculateBins_Globally_RevK(YYY,XXX,Area,Nbin,Nsurface,...
...
%% here comes a difference with RevH (Also ... have taken away some of the
%% area)

GoodArea=AreaFilter(Area,Area);
...
if debugplots==1
    ...
    grid on
    ylims=get(gca,'YLim');
    title([' Contour-filtered areas. ' cropname ' ' WetFlag ' RevK']);
    fattenplot
    shading flat
    finemap('area2')
    OutputFig('Force')
    ...
    zeroylim(ylims(1),ylims(2));
    grid on
    title([' 95% Contour ' cropname ' ' WetFlag ' RevK']);
    fattenplot
    shading flat
    finemap('jet')
    OutputFig('Force')
...
% cheesy step to exclude non-valid data points (i.e. the 5% of small area
% points)
x(~IsValidData)=0;
y(~IsValidData)=0;
Area(~IsValidData)=0;
