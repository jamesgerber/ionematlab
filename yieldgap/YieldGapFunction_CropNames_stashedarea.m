...
%  FS.csqirev='Ar1';
%
%  stashedarea - sep 28, 2015.  now that I'm doing multi-year yieldgaps, I
%  have problems with the code inserting the year 2000 areas.  This version
%  will 'stash' the year-specific areas and then unstash after loading in
%  the year 2000 bins.
    ...
    for j=1:Ncrop
        for m=1:Nclim;           
            for k=1:NPercentileForMaxYield
                ...
                OutputStructure=YieldGapFunction_CropNames_stashedarea(FS);
                end
            end
        end
    end
...
stash CultivatedArea
load([ClimateLibraryDir '/' ClimateMaskFile]);
unstash CultivatedArea

if exist('BinMatrix')==0
    BinMatrix=datablank;
