...
% OutputStructure=MakeClimateSpaceLibraryFunctionRevJ(FS);
%
%
%
...
for N=Nspace;
    for jcrop=jcropvector
     ...
     %   LogicalKeep=AreaFilter(CultivatedArea,CultivatedArea);
     %   CultivatedArea(~LogicalKeep)=0;
        ...
        if exist([FileName '.nc'])==2
            ...
            disp(['Making bins'])
                ...
                % RevJ - if Cultivated Area ==0, then do not take those
                ...
                [PrecBinEdges,GDDBinEdges,xbins,ybins,ContourMask]= ...
                    CalculateBins_Globally_RevJ(Prec,tempvar,...
          ...
          %  GDDBinEdges
            ...
            disp('refining bins')
            [CDSnew]=...
                RefineClimateSpaceRevJ(Heat,Prec,CultivatedArea,CDS,xbins,ybins,ContourMask,[cropname ' ' WetFlag]);
